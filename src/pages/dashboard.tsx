...
import { DesignListPage as ShowCaseList } from 'pages/designs';
...
function DashboardPage() {
  ...
  const experimentStat = useSWR<PaginatedInterface<ExperimentInterface>>(
    () => `/experiments`,
    () => getExperiments({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Experiments',
      result: experimentStat?.data?.totalCount,
      isLoading: experimentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`designs`} passHref legacyBehavior>
                  <Button
