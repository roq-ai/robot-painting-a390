import {
  FormControl,
  FormLabel,
  Input,
  Button,
  Text,
  Box,
  Spinner,
  FormErrorMessage,
  Switch,
  Flex,
} from '@chakra-ui/react';
import Breadcrumbs from 'components/breadcrumb';
import DatePicker from 'components/date-picker';
import { Error } from 'components/error';
import { FormWrapper } from 'components/form-wrapper';
import { NumberInput } from 'components/number-input';
import { SelectInput } from 'components/select-input';
import { AsyncSelect } from 'components/async-select';
import { TextInput } from 'components/text-input';
import AppLayout from 'layout/app-layout';
import { FormikHelpers, useFormik } from 'formik';
import { useRouter } from 'next/router';
import { FunctionComponent, useState } from 'react';
import * as yup from 'yup';
import { AccessOperationEnum, AccessServiceEnum, requireNextAuth, withAuthorization } from '@roq/nextjs';
import { compose } from 'lib/compose';

import { createDesign } from 'apiSdk/designs';
import { designValidationSchema } from 'validationSchema/designs';
import { ExperimentInterface } from 'interfaces/experiment';
import { UserInterface } from 'interfaces/user';
import { getExperiments } from 'apiSdk/experiments';
import { getUsers } from 'apiSdk/users';
import { DesignInterface } from 'interfaces/design';

function DesignCreatePage() {
  const router = useRouter();
  const [error, setError] = useState(null);

  const handleSubmit = async (values: DesignInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createDesign(values);
      resetForm();
      router.push('/designs');
    } catch (error) {
      setError(error);
    }
  };

  const formik = useFormik<DesignInterface>({
    initialValues: {
      design_name: '',
      creation_date: new Date(new Date().toDateString()),
      modification_date: new Date(new Date().toDateString()),
      status: '',
      experiment_id: (router.query.experiment_id as string) ?? null,
      user_id: (router.query.user_id as string) ?? null,
    },
    validationSchema: designValidationSchema,
    onSubmit: handleSubmit,
    enableReinitialize: true,
    validateOnChange: false,
    validateOnBlur: false,
  });

  return (
    <AppLayout
      breadcrumbs={
        <Breadcrumbs
          items={[
            {
              label: 'Designs',
              link: '/designs',
            },
            {
              label: 'Create Design',
              isCurrent: true,
            },
          ]}
        />
      }
    >
      <Box rounded="md">
        <Box mb={4}>
          <Text as="h1" fontSize={{ base: '1.5rem', md: '1.875rem' }} fontWeight="bold" color="base.content">
            Create Design
          </Text>
        </Box>
        {error && (
          <Box mb={4}>
            <Error error={error} />
          </Box>
        )}
        <FormWrapper onSubmit={formik.handleSubmit}>
          <TextInput
            error={formik.errors.design_name}
            label={'Design Name'}
            props={{
              name: 'design_name',
              placeholder: 'Design Name',
              value: formik.values?.design_name,
              onChange: formik.handleChange,
            }}
          />

          <FormControl id="creation_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Creation Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.creation_date ? new Date(formik.values?.creation_date) : null}
              onChange={(value: Date) => formik.setFieldValue('creation_date', value)}
            />
          </FormControl>
          <FormControl id="modification_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Modification Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.modification_date ? new Date(formik.values?.modification_date) : null}
              onChange={(value: Date) => formik.setFieldValue('modification_date', value)}
            />
          </FormControl>

          <TextInput
            error={formik.errors.status}
            label={'Status'}
            props={{
              name: 'status',
              placeholder: 'Status',
              value: formik.values?.status,
              onChange: formik.handleChange,
            }}
          />

          <AsyncSelect<ExperimentInterface>
            formik={formik}
            name={'experiment_id'}
            label={'Select Experiment'}
            placeholder={'Select Experiment'}
            fetcher={getExperiments}
            labelField={'experiment_name'}
          />
          <AsyncSelect<UserInterface>
            formik={formik}
            name={'user_id'}
            label={'Select User'}
            placeholder={'Select User'}
            fetcher={getUsers}
            labelField={'email'}
          />
          <Flex justifyContent={'flex-start'}>
            <Button
              isDisabled={formik?.isSubmitting}
              bg="state.info.main"
              color="base.100"
              type="submit"
              display="flex"
              height="2.5rem"
              padding="0rem 1rem"
              justifyContent="center"
              alignItems="center"
              gap="0.5rem"
              mr="4"
              _hover={{
                bg: 'state.info.main',
                color: 'base.100',
              }}
            >
              Submit
            </Button>
            <Button
              bg="neutral.transparent"
              color="neutral.main"
              type="button"
              display="flex"
              height="2.5rem"
              padding="0rem 1rem"
              justifyContent="center"
              alignItems="center"
              gap="0.5rem"
              mr="4"
              onClick={() => router.push('/designs')}
              _hover={{
                bg: 'neutral.transparent',
                color: 'neutral.main',
              }}
            >
              Cancel
            </Button>
          </Flex>
        </FormWrapper>
      </Box>
    </AppLayout>
  );
}

export default compose(
  requireNextAuth({
    redirectTo: '/',
  }),
  withAuthorization({
    service: AccessServiceEnum.PROJECT,
    entity: 'design',
    operation: AccessOperationEnum.CREATE,
  }),
)(DesignCreatePage);
